    ...
    implements Cascade_Driver_KVS
{
        ...
        function is_enable(/* boolean */ $notice = TRUE)
    {
        ...
        if (extension_loaded($extension_name)
            && $required_version <= phpversion($extension_name)) {
            return TRUE;
        }
        // 警告表示
        ...
        if ($is_success) {
            ...
            if ($is_success === FALSE) {
                if (apc_exists($storage_key) === FALSE) {
                    ...
                    $this->errno_code    = self::RES_NOTFOUND;
                    $this->error_message = sprintf($error, $this->namespace, $key);
                } else {
                    $error = 'Faild to get stored value {namespace, key} %s %s';
                    $this->errno_code    = self::RES_FAILURE;
                    $this->error_message = sprintf($error, $this->namespace, $key);
                }
            }
        }
                     ...
                     /* int    */ $expiration = NULL)
    {
        ...
        if ($is_success) {
            if (apc_exists($storage_key) === FALSE) {
                ...
                $this->errno_code    = self::RES_DATA_EXISTS;
                $this->error_message = sprintf($error, $this->namespace, $key);
            }
        }
