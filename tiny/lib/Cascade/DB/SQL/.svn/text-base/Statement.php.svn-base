    ...
    extends Cascade_DB_Statement
{
        ...
        function prepareForGet(/* void */)
    {
        ...
        $this->prosessQuery($query, $params);
    }
    // }}}
    // {{{ prepareForMultiGet
    /**
        ...
        function perform(/* void */)
    {
        ...
        case Cascade_DB_SQL_Criteria::TYPE_IS_VALUE:
        case Cascade_DB_SQL_Criteria::TYPE_IS_EXEC:
            if ($is_success = $driver->query($this->query, $this->bind_values)) {
     ...
     */
    protected /* void */
        function prosessQuery(/* string */ $query,
                              /* array  */ $params)
    {
        // 文字列タイプのプレースホルダを抽出する
        $pholder = array();
                                ...
                                /* array */ $params)
    {
        ...
        foreach ($pholder[self::P_HOLDER_BIND] as $bind) {
            ...
            if (array_key_exists($b_name, $params) === FALSE) {
                ...
                $ex_msg  = sprintf($ex_msg, get_class($this->data_format), $_key);
                throw new Cascade_Exception_DBException($ex_msg);
            }
                              ...
                              /* int   */ $type = self::VAR_TYPE_IS_STRING)
    {
        ...
        case self::VAR_TYPE_IS_STRING:
        {
            ...
            case self::VAR_TYPE_IS_STRING:
                ...
                break;
            default:
                // OTHER Z_TYPE (IS_ARRAY, IS_RESOURCE, ...)
                $ex_msg = 'Invalid Z_TYPE of bind-value {z_type, value} %s %s';
                $ex_msg = sprintf($ex_msg, gettype($value), var_export($value, TRUE));
                throw new Cascade_Exception_DBException($ex_msg);
            }
            break;
        }
        default:
            ...
            $ex_msg = sprintf($ex_msg, $type, var_export($value, TRUE));
            throw new Cascade_Exception_DBException($ex_msg);
        }
        // 内部変数に保存
        ...
        function getDriver(/* void */)
    {
        ...
        if ($this->driver === NULL) {
            ...
            $dsn  = $this->getDataFormat()->getActiveDSN($this->getCriteria());
            $args = array($dsn);
            $this->driver = Cascade_Driver_Factory::getInstance($type, $args);
            if ($this->driver->get_error_code()) {
                $ex_msg = $this->driver->get_error_message();
