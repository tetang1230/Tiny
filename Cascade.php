...
final class Cascade
{
    ...
    public static /* void */
        function registerAutoload(/* string */ $dir_path,
                                  ...
                                  /* string */ $file_ext = '.php')
    {
        AutoLoad::register($dir_path, $prefix, $file_ext);
    }
    // }}}
    // ----[ Class Constants ]----------------------------------------
    // {{{ SESSION
...
if (!defined('CASCADE_SRC_ROOT')) {
      define('CASCADE_SRC_ROOT', dirname(__FILE__).'/tiny/lib');
}
...
if (!defined('CLASS_SRC_ROOT')) {
      define('CLASS_SRC_ROOT', dirname(__FILE__).'/class');
}

Cascade::registerAutoload(CLASS_SRC_ROOT);
// ----[ Autoload ]---------------------------------------------------
// Autoloaderの設定
